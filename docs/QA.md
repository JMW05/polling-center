...
(`cf15b38`) and in the ES-module extraction (`aec3444`), so none is a
regression from the module extraction. The last two were fixed afterwards in a
separate commit (startup/auth render fix); the first two are still open.
